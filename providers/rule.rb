...
action :append do
  ...
  if not node["simple_iptables"]["chains"][new_resource.table].include?(new_resource.chain)
    node["simple_iptables"]["chains"][new_resource.table] << new_resource.chain
    node["simple_iptables"]["rules"][new_resource.table] << "-A #{new_resource.direction} --jump #{new_resource.chain}"
  end
  ...
  rules.each do |rule|
    ...
    if not node["simple_iptables"]["rules"][new_resource.table].include?(new_rule)
      node["simple_iptables"]["rules"][new_resource.table] << new_rule
